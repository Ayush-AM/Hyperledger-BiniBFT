...
func (c *Controller) leaderID() uint64 {
	// For hierarchical consensus, return the PRIMARY node instead of traditional leader
	return GetPrimaryNode(c.getCurrentViewNumber(), c.N, c.NodesList, c.getCurrentDecisionsInView(), c.DecisionsPerLeader)
}
...
func (c *Controller) BroadcastConsensus(m *protos.Message) {
	...
	if m.GetPrePrepare() != nil || m.GetPrepare() != nil || m.GetCommit() != nil {
		// In hierarchical consensus, both PRIMARY and SECONDARY nodes can send heartbeats
		nodesList := c.Comm.Nodes()
		myRole := GetNodeRole(c.ID, c.getCurrentViewNumber(), c.N, nodesList, c.getCurrentDecisionsInView(), c.DecisionsPerLeader)
